...
function RepertoireWorkspace() {
    ...
    if (drill) return <main className={hubStyles.page}><RepertoireDrill store={store} repertoireId={drill.repertoireId} mixed={drill.mixed} onExit={() => setDrill(undefined)}/></main>;
    ...
    if (target && mode == "mine") return <main className={hubStyles.page}>{panel(target)}</main>;

    return <main className={hubStyles.page}>
        {!courseFocused && <><section className={hubStyles.hero}><div><span className={hubStyles.eyebrow}>{t("eyebrow")}</span><h1>{t("title")}</h1><p className={hubStyles.heroText}>{t("intro")}</p></div><div className={hubStyles.explainer}><strong>{t("whatIs.title")}</strong><p>{t("whatIs.body")}</p><span>{t("whatIs.reassurance")}</span></div></section><nav className={hubStyles.modeNav} aria-label={t("modes.label")}>{(["mine","learn","review","import"] as WorkspaceMode[]).map(item => <button type="button" key={item} className={mode == item ? hubStyles.modeActive : hubStyles.modeButton} onClick={() => changeMode(item)}><span className={hubStyles.modeIcon}>{item == "mine" ? "♙" : item == "learn" ? "▤" : item == "review" ? "↻" : "⇩"}</span><strong>{t(`modes.${item}`)}</strong><small>{t(`modes.${item}Help`)}</small></button>)}</nav></>}
