...
function buildModel(lines:OpeningCatalogueEntry[]):TreeModel{
 ...
 const byId=new Map<string,TreeNode>([[root.id,root]]),byPosition=new Map<string,TreeNode[]>([[root.positionKey,[root]]]),entryNodes=new Map<string,TreeNode>();
...
function connectorPath(index:number,count:number){const x=(index+.5)*(1000/count);return `M500 0 C500 24 ${x} 24 ${x} 52`;}
...
function CourseRepertoireTree({name,lines,progress,preferredSide,percent,language,title,loadingLabel,onOpen,onAddToRepertoire}:Props){
 ...
 const[focusId,setFocusId]=useState("root"),[query,setQuery]=useState(""),[extraVisible,setExtraVisible]=useState(0),[fullscreen,setFullscreen]=useState(false),[semanticLevel,setSemanticLevel]=useState(1),[explorer,setExplorer]=useState<OpeningExplorerPosition|null|undefined>(),[added,setAdded]=useState<Set<string>>(()=>new Set());
 const focus=model.byId.get(focusId)||model.root,currentPath=pathNodes(focus),routesHere=(model.byPosition.get(focus.positionKey)||[]).filter(node=>node.id!=focus.id),effectiveLevel=fullscreen?semanticLevel:1;

 useEffect(()=>{setFocusId("root");setQuery("");setExtraVisible(0);setSemanticLevel(1);},[name]);
 ...
 const bonus=effectiveLevel==0?-2:effectiveLevel==2?2:effectiveLevel==3?5:0,visibleCount=Math.max(3,Math.min(records.length,NORMAL_LIMIT+bonus+extraVisible)),visible=records.slice(0,visibleCount),hidden=Math.max(0,records.length-visibleCount);
 ...
 const arrows=visible.slice(0,5).flatMap((record,index)=>record.node.uci.length<4?[]:[{from:record.node.uci.slice(0,2) as Square,to:record.node.uci.slice(2,4) as Square,colour:ARROW_COLOURS[index],overlayColour:ARROW_COLOURS[index]}]);
 const exact=exactStudyEntry(focus,progress),top=visible[0],boardOrientation:RepertoireSide=preferredSide=="black"?"black":"white";

 function focusNode(node:TreeNode){setFocusId(node.id);setExtraVisible(0);setQuery("");}
 ...
 return <div ref={cardRef} className={styles.card} data-repertoire-tour="focus-tree">
  ...
  <nav className={styles.breadcrumb} aria-label={copy.current}><button type="button" data-current={focus==model.root} onClick={()=>focusNode(model.root)}>{localizeOpeningName(name,language)}</button>{currentPath.map(node=><React.Fragment key={node.id}><span>›</span><button type="button" data-current={node.id==focus.id} onClick={()=>focusNode(node)}>{moveLabel([node])}</button></React.Fragment>)}</nav>
  <main className={styles.workspace} data-level={effectiveLevel}>
   <section className={styles.boardPanel}><div className={styles.boardHeading}><span>{copy.current}</span><strong>{currentTitle}</strong><small>{currentPath.length?moveLabel(currentPath.slice(-8)):copy.courseStart}</small></div><div className={styles.boardWrap}><Chessboard id="repertoire-course-tree-board" position={focus.fen} boardOrientation={boardOrientation} arePiecesDraggable={false} customPieces={pieces} customDarkSquareStyle={{backgroundColor:settings.themes.board.darkSquareColour}} customLightSquareStyle={{backgroundColor:settings.themes.board.lightSquareColour}} showBoardNotation={settings.themes.board.coordinates=="inside"}/><SuggestionArrowOverlay arrows={arrows} flipped={boardOrientation=="black"}/></div><div className={styles.positionMeta}>{explorer===undefined?<span>{copy.loadingStats}</span>:explorer===null?<span>{copy.noStats}</span>:<span>{copy.commonHelp}</span>}</div></section>
   <section className={styles.treePanel}>
    <div className={styles.currentNode}><span>{copy.current}</span><strong>{currentPath.length?moveLabel(currentPath.slice(-4)):currentTitle}</strong><small>{focus.descendantLines} {copy.courseCoverage}</small>{routesHere.length>0&&<em>↔ {copy.transposition} · {routesHere.length+1} {copy.routes}</em>}</div>
    {routesHere.length>0&&<div className={styles.transpositions}><div><strong>↔ {copy.transposition}</strong><span>{copy.transpositionHelp}</span></div><div>{routesHere.slice(0,3).map(route=><button type="button" key={route.id} onClick={()=>focusNode(route)}>{moveLabel(pathNodes(route).slice(-5))}</button>)}</div></div>}
    {top&&<div className={styles.prompt}><span>{copy.commonNext}</span><strong>{top.share!=null?`${Math.round(top.share)}% ${copy.games}: ${moveLabel([top.node])}`:`${moveLabel([top.node])} · ${top.node.descendantLines} ${copy.courseCoverage}`}</strong><p>{copy.popularPrompt}</p></div>}
    {visible.length>0&&<div className={styles.treeFan}><svg className={styles.connectors} viewBox="0 0 1000 52" preserveAspectRatio="none" aria-hidden="true">{visible.map((record,index)=><path key={record.node.id} d={connectorPath(index,visible.length)}/>)}</svg><div className={styles.children} style={{gridTemplateColumns:`repeat(${visible.length}, minmax(0, 1fr))`}} data-repertoire-tour="focus-branches">{visible.map((record,index)=>{const node=record.node,routeCount=model.byPosition.get(node.positionKey)?.length||1,studyEntry=exactStudyEntry(node,progress),liveText=record.share!=null?`${Math.round(record.share)}% ${copy.games}`:`${node.descendantLines} ${copy.courseCoverage}`;return <article key={node.id} className={styles.branchCard} style={{"--branch-colour":ARROW_COLOURS[index%ARROW_COLOURS.length]} as React.CSSProperties}><div className={styles.branchTop}><span>{moveLabel([node])}</span><b>{liveText}</b></div><strong>{branchName(node,language,node.san)}</strong><small>{node.descendantLines} {copy.lines}</small>{routeCount>1&&<em>↔ {copy.transposition} · {routeCount}</em>}<div className={styles.branchActions}><button type="button" onClick={()=>focusNode(node)}>{copy.explore}</button><button type="button" data-primary disabled={added.has(node.id)} onClick={()=>addBranch(node)}>{added.has(node.id)?`✓ ${copy.added}`:`＋ ${copy.add}`}</button>{studyEntry&&<button type="button" data-study onClick={()=>study(studyEntry)}>{copy.study}</button>}</div></article>;})}</div></div>}
    {hidden>0&&<button type="button" className={styles.moreButton} onClick={()=>setExtraVisible(value=>value+MORE_STEP)}>＋ {copy.showMore} ({hidden})</button>}{extraVisible>0&&<button type="button" className={styles.lessButton} onClick={()=>setExtraVisible(0)}>↑ {copy.showLess}</button>}
