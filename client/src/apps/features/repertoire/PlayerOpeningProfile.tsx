...
import {
    PlayerOpeningProfile as PlayerOpeningProfileResult,
    PlayerOpeningStat,
    PlayerPlatform,
    PlayerSide,
...
interface PlayerOpeningProfileProps {
    ...
    onTrainFamily: (family: string, side: PlayerSide) => void;
}
...
function ProfileList({
    ...
    title: string;
    items: PlayerOpeningStat[];
    emptyLabel: string;
}) {
    return <div className={styles.repertoireColumn}>
        <strong>{title}</strong>
        {items.length == 0
            ? <p>{emptyLabel}</p>
            : items.slice(0, 4).map(item => <div
                key={`${item.side}-${item.family}`}
                className={styles.observedLine}
            >
                <span>{item.family}</span>
                <small>{item.games} · {scoreLabel(item.scoreRate)}</small>
            </div>)}
    </div>;
}

function PlayerOpeningProfile({
    catalogue,
    onTrainFamily
    ...
    async function analyse(event: React.FormEvent) {
        ...
        setError("");
        try {
            setProfile(await analysePlayerOpenings(
                platform,
                username,
                maximum,
                catalogue
