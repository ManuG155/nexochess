...
function scoreLabel(value: number) { return new Intl.NumberFormat(undefined, { style: "percent", maximumFractionDigits: 0 }).format(value); }
function ProfileList({ title, items, emptyLabel, language }: { title: string; items: PlayerOpeningStat[]; emptyLabel: string; language: string; }) {
    return <div className={styles.repertoireColumn}><strong>{title}</strong>{items.length == 0 ? <p>{emptyLabel}</p> : items.slice(0, 6).map(item => <div key={`${item.side}-${item.family}`} className={styles.observedLine}><span>{localizeOpeningName(item.family, language)}</span><small>{item.games} · {scoreLabel(item.scoreRate)}</small></div>)}</div>;
}
function PlayerOpeningProfile({ catalogue, onTrainFamily }: PlayerOpeningProfileProps) {
    const { t, i18n } = useTranslation("repertoireCourse");
    ...
    async function analyse(event: React.FormEvent) {
        event.preventDefault(); if (!username.trim() || loading) return; setLoading(true); setError("");
    ...
    return <section className={styles.profileCard}>
        ...
        {expanded && <><form className={styles.profileForm} onSubmit={analyse}>
        ...
        </form>{error && <p className={styles.error}>{error}</p>}{profile && <div className={styles.profileResult}>
            ...
            {recommendation && <div className={styles.recommendation}><div><span>{t("profile.startHere")}</span><strong>{localizeOpeningName(recommendation.family, language)}</strong><p>{t("profile.recommendation", { side: tRepertoire(`side.${recommendation.side}`), games: recommendation.games, score: scoreLabel(recommendation.scoreRate), moves: recommendation.averageKnownMoves })}</p></div><button type="button" onClick={() => onTrainFamily(recommendation.family, recommendation.side)}>{t("profile.train")}</button></div>}
            <div className={styles.observedGrid}><ProfileList title={t("profile.whiteRepertoire")} items={profile.white} emptyLabel={t("profile.noData")} language={language}/><ProfileList title={t("profile.blackRepertoire")} items={profile.black} emptyLabel={t("profile.noData")} language={language}/></div>
        </div>}</>}
    </section>;
}
