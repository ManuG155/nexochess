...
import { savePositionHabitsFromGames } from "./playerPositionHabits";
...
async function fetchChessComGames(username: string, maximum: number) {
    const archiveResponse = await fetch(`https://api.chess.com/pub/player/${encodeURIComponent(username)}/games/archives`, { headers: { Accept: "application/json" } });
    ...
    for (const archive of archives) {
        ...
        const response = await fetch(archive, { headers: { Accept: "application/json" } });
        ...
        const games = [...(payload.games || [])].filter(game => game.rules == null || game.rules == "chess").sort((a, b) => (b.end_time || 0) - (a.end_time || 0));
        for (const game of games) {
            if (result.length >= maximum) break;
            if (!game.pgn) continue;
            const whiteName = game.white?.username?.toLocaleLowerCase();
            const blackName = game.black?.username?.toLocaleLowerCase();
            const side: PlayerSide | undefined = whiteName == normalizedUsername ? "white" : blackName == normalizedUsername ? "black" : undefined;
            if (!side) continue;
            const moves = parsePgnMoves(game.pgn);
            if (moves.length == 0) continue;
            const player = side == "white" ? game.white : game.black;
            result.push({ moves, side, score: scoreFromChessComResult(player?.result || "") });
        }
    }
...
async function fetchLichessGames(username: string, maximum: number) {
    ...
    for (const line of (await response.text()).split(/\r?\n/)) {
        ...
        const whiteName = (game.players?.white?.user?.id || game.players?.white?.user?.name || "").toLocaleLowerCase();
        const blackName = (game.players?.black?.user?.id || game.players?.black?.user?.name || "").toLocaleLowerCase();
        const side: PlayerSide | undefined = whiteName == normalizedUsername ? "white" : blackName == normalizedUsername ? "black" : undefined;
        const moves = game.moves?.trim().split(/\s+/).filter(Boolean) || [];
        if (!side || moves.length == 0) continue;
        result.push({ moves, side, score: game.winner == null ? 0.5 : game.winner == side ? 1 : 0 });
    }
    if (result.length == 0) throw new Error("no-games");
    return result;
}
...
function buildStats(games: RecentGame[], catalogue: OpeningCatalogueEntry[]) {
    ...
    const groups = new Map<string, { family: string; side: PlayerSide; games: number; wins: number; draws: number; losses: number; points: number; knownPlies: number }>();
    for (const game of games) {
        const match = classifyGame(game, matchers);
        const family = match?.opening.family || "Other openings";
        const key = `${game.side}|${family}`;
        const group = groups.get(key) || { family, side: game.side, games: 0, wins: 0, draws: 0, losses: 0, points: 0, knownPlies: 0 };
        group.games += 1;
    ...
    return Array.from(groups.values()).map(group => {
        ...
        const averageKnownMoves = group.games > 0 ? Math.round((group.knownPlies / group.games / 2) * 10) / 10 : 0;
        const priority = group.games * (1.15 - scoreRate) + Math.max(0, 4 - averageKnownMoves) * 0.2;
        return { family: group.family, side: group.side, games: group.games, wins: group.wins, draws: group.draws, losses: group.losses, scoreRate, averageKnownMoves, priority } satisfies PlayerOpeningStat;
    });
}

export async function analysePlayerOpenings(platform: PlayerPlatform, username: string, maximum: number, catalogue: OpeningCatalogueEntry[]): Promise<PlayerOpeningProfile> {
    const cleanUsername = username.trim();
    if (!cleanUsername) throw new Error("invalid-username");
    const games = platform == "chesscom" ? await fetchChessComGames(cleanUsername, maximum) : await fetchLichessGames(cleanUsername, maximum);
    savePositionHabitsFromGames(games, platform, cleanUsername);
    const stats = buildStats(games, catalogue);
    const byFrequency = (side: PlayerSide) => stats.filter(item => item.side == side && item.family != "Other openings").sort((a, b) => b.games - a.games || a.scoreRate - b.scoreRate);
    const recommendations = stats.filter(item => item.family != "Other openings").sort((a, b) => b.priority - a.priority || b.games - a.games).slice(0, 3);
    return {
