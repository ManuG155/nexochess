...
assert.equal(Date.parse(cutoff), Date.parse("2026-08-21T22:00:00Z"));
...
for (const language of languages) {
    assert.ok(
        ...
        `Missing v1.3 release-note copy for ${language}.`
    );
}
...
for (const fragment of [
    ...
    assert.ok(releaseNotice.includes(fragment), `v1.3 release notice is missing: ${fragment}`);
}

assert.ok(
    !releaseNotice.includes("/api/") && !releaseNotice.includes("credentials:"),
    "The v1.3 notice must remain browser/device-local and independent of account state."
);
assert.ok(
...
assert.ok(
    pageWrapper.includes('lazy(() => import("@/components/releases/ReleaseNoticeV13"))'),
    "The v1.3 release notice must be loaded globally and lazily."
);
assert.ok(pageWrapper.includes("<ReleaseNoticeV13/>"));

const scripts = JSON.parse(packageJson).scripts;
assert.equal(scripts["verify:release-v1.3"], "node scripts/verify-release-v1.3.mjs");
assert.ok(
    scripts.check.includes("npm run verify:release-v1.3"),
    "The v1.3 release audit must run in the main check pipeline."
);

console.log(
    "NexoChess v1.3 release verification passed: 11 localized notes, "
    + "one-time browser/device persistence and the 21 August 23:59 CEST cutoff are wired."
);
